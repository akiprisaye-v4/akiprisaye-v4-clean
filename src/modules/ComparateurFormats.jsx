    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {formatsAvecPrixUnitaire.map((format, index) => {
          ...
          const isPire = format.prixUnitaire === pireFormat.prixUnitaire && formatsAvecPrixUnitaire.length > 1;
          const ecartPourcent = ((format.prixUnitaire - meilleurFormat.prixUnitaire) / meilleurFormat.prixUnitaire * 100);
            ...
            >
              ...
              <div className="p-3 bg-slate-100 dark:bg-slate-800 rounded-lg mb-4">
                <p className="text-sm font-mono text-gray-700 dark:text-gray-300">
                  ...
                  <br />
                  = {format.prix.toFixed(2)} € ÷ {format.quantite} {format.unite}
                  <br />
                  = {format.prixUnitaire.toFixed(2)} €/{format.unite}
                </p>
              </div>
              ...
              {!isMeilleur && ecartPourcent > 0.01 && (
                <div className={`p-4 rounded-lg ${
                  ...
                  <p className={`text-sm font-medium ${
                    ...
                    📊 Ce format est <strong>{ecartPourcent.toFixed(1)}% plus cher</strong> au {format.unite} 
                  ...
                  <p className={`text-xs mt-2 ${
                    ...
                    Différence : +{(format.prixUnitaire - meilleurFormat.prixUnitaire).toFixed(2)} €/{format.unite}
                  </p>
                </div>
              )}
      ...
      <Card className="p-6 bg-blue-50 dark:bg-blue-900/20">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="text-sm text-gray-600 dark:text-gray-400">
                Prix au {meilleurFormat.unite} : {meilleurFormat.prixUnitaire.toFixed(2)} € • 
                Total : {meilleurFormat.prix.toFixed(2)} €
              </div>
            </div>
          </div>

          {formatsAvecPrixUnitaire.length > 1 && meilleurFormat.prixUnitaire !== pireFormat.prixUnitaire && (
            <div className="flex items-start gap-3">
              <span className="text-2xl">⚠️</span>
              <div>
                <div className="font-semibold text-gray-900 dark:text-white">
                ...
                <div className="text-sm text-gray-600 dark:text-gray-400">
                  Prix au {pireFormat.unite} : {pireFormat.prixUnitaire.toFixed(2)} € • 
                  Surcoût de {((pireFormat.prixUnitaire - meilleurFormat.prixUnitaire) / meilleurFormat.prixUnitaire * 100).toFixed(1)}%
                </div>
              </div>
            </div>
          )}
        ...
        <div className="mt-4 pt-4 border-t border-blue-200 dark:border-blue-800">
          <p className="text-sm text-gray-700 dark:text-gray-300">
            <strong>💡 Principe :</strong> Vérifiez toujours le prix au kilo ou au litre.
            Le format le plus gros n'est pas toujours le plus économique.
          </p>
        </div>
      </Card>
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
        <p className="text-sm text-blue-800 dark:text-blue-200">
          ℹ️ Cette analyse est purement mathématique. Aucune enseigne n'est nommée.
          Les prix sont fournis à titre d'exemple pédagogique.
        </p>
      </div>
    </div>
  );
